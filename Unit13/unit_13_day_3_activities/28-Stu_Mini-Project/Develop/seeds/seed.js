...
const seedDatabase = async () => {
  ...
  for (let i = 0; i < 10; i += 1) {
    ...
    // Create a new trip with random `trip_budget` and `traveller_amount`
    // values, but with ids selected above
    trips.push({
  ...
  // If there's an error, such as the same random pairing of `traveller.id` and
  // `location.id` occurring and we get a constraint error, don't quit the Node
  // process
  await Promise.all(
    trips.map(trip => Trip.create(trip).catch(error => console.log(error)))
  );
