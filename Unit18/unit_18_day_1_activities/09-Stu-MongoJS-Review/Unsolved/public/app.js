...
function getUnread() {
  ...
  $.getJSON('/unread', function (data) {
    for (let i = 0; i < data.length; i++) {
      $('#unread').prepend(
        `<tr><td>${data[i].title}</td><td>${data[i].author}</td><td><button class='markread' data-id='${data[i]._id}'>Mark Read</button></td></tr>`
      );
    }
    $('#unread').prepend(
...
function getRead() {
  ...
  $.getJSON('/read', function (data) {
    for (let i = 0; i < data.length; i++) {
      $('#read').prepend(
        `<tr><td>${data[i].title}</td><td>${data[i].author}</td><td><button class='markunread' data-id='${data[i]._id}'>Mark Unread</button></td></tr>`
      );
    }
    $('#read').prepend(
      '<tr><th>Title</th><th>Author</th><th>Read/Unread</th></tr>'
