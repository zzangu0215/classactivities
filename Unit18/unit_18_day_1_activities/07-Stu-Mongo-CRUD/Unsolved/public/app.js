function getResults() {
  ...
  $.getJSON('/all', function (data) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      $('#results').prepend(
        `<p class='data-entry' data-id=${data[i]._id}><span class='dataTitle' data-id=${data[i]._id}>${data[i].title}</span><span class='delete'>X</span></p>`
      );
    }
  });
}
...
$(document).on('click', '#make-new', function () {
  ...
  $.ajax({
    ...
    .then(function (data) {
      ...
      $('#results').prepend(
        `<p class='data-entry' data-id=${data._id}><span class='dataTitle' data-id=${data._id}>${data.title}</span><span class='delete'>X</span></p>`
      );
